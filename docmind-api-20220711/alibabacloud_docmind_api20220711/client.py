...
class Client(OpenApiClient):
    ...
    *\
    ...
    def aync_trade_document_package_extract_smart_app_with_options(self, tmp_req, runtime):
    ...
    def aync_trade_document_package_extract_smart_app(self, request):
    ...
    def get_page_num_with_options(self, request, runtime):
        UtilClient.validate_model(request)
        query = {}
    ...
    def submit_digital_doc_structure_job_with_options(self, request, runtime):
        UtilClient.validate_model(request)
        query = {}
        if not UtilClient.is_unset(request.file_name):
            query['FileName'] = request.file_name
    ...
    def submit_digital_doc_structure_job(self, request):
        runtime = util_models.RuntimeOptions()
        return self.submit_digital_doc_structure_job_with_options(request, runtime)

    def submit_digital_doc_structure_job_advance(self, request, runtime):
        # Step 0: init client
        access_key_id = self._credential.get_access_key_id()
        access_key_secret = self._credential.get_access_key_secret()
        security_token = self._credential.get_security_token()
        ...
        if not UtilClient.is_unset(request.file_url_object):
            ...
            oss_client.post_object(upload_request, oss_runtime)
            submit_digital_doc_structure_job_req.file_url = 'http://%s.%s/%s' % (TeaConverter.to_unicode(auth_response.body.bucket), TeaConverter.to_unicode(auth_response.body.endpoint), TeaConverter.to_unicode(auth_response.body.object_key))
